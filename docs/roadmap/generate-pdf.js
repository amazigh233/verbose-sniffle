...
async function main() {
  const source = path.join(__dirname, "functionaliteiten-roadmap.html");
  const output = path.join(__dirname, "Climature-Functionaliteiten-Roadmap.pdf");
  ...
  await page.pdf({ path: output, format: "A4", printBackground: true, preferCSSPageSize: true, margin: { top: 0, right: 0, bottom: 0, left: 0 }, tagged: true, outline: true });
  await browser.close();
