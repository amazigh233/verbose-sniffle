...
const schemas = {
  ...
  products: base.extend({ category: z.string().trim().min(1).max(100), brand: z.string().max(100), name: z.string().trim().min(1).max(180), specs: z.string().max(1000).optional(), priceExVat: money, vatRate: money, description: z.string().max(4000).optional() }).passthrough(),
