(function () {
  ...
  function technologyCard(kind, result) {
    ...
    return '<article class="advice-v2-tech ' + (eligible ? 'is-ready' : 'is-blocked') + '"><div class="advice-v2-tech-head"><div><p class="eyebrow">' + (kind === 'warmtepomp' ? 'Warmtepomp' : 'Thuisbatterij') + '</p><h3>' + esc(result.label) + '</h3></div><strong class="advice-v2-status">' + esc(statusLabel(result.status)) + '</strong></div>' +
      ...
      (product ? '<p class="advice-v2-product"><strong>' + esc(product.name) + '</strong>' + (kind === 'warmtepomp' && result.subsidy ? '<span>Indicatieve ISDE: ' + money(result.subsidy) + '</span>' : '') + '</p>' : '') +
  ...
  function assumptionsHtml(result) {
    ...
    return '<details class="panel advice-v2-assumptions"><summary>Berekening, bronnen en voorbehoud</summary><span>Gas ' + money(a.gasPrice) + '/m³ · stroom ' + money(a.electricityPrice) + '/kWh · teruglevering ' + money(a.feedInCost) + '/kWh · EPEX-marge ' + money(a.epexMargin) + '/kWh</span><small>' + esc(labels.join(' · ') || 'Centrale portaalinstellingen') + '</small><small>Rekenversie ' + esc(result.engineVersion) + ' · ' + esc(new Date(result.calculatedAt).toLocaleString('nl-NL')) + '. Definitieve dimensionering volgt na technische opname.</small></details>';
  }
  ...
  function calculate() {
    saveDraft(); state.result = E.calculate(collect(), S.settings().adviceAssumptions || {}); state.draftInput = state.result.input;
    ...
    state.customerId = state.result.input.customerId || state.customerId; state.step = 4; showStep(4);
  }
  ...
  function quoteLines(selected) {
    ...
    if (selected.indexOf('warmtepomp') >= 0 && wp && wp.product) lines.push({ description: wp.product.name + ' · advies ' + decimal(wp.requiredKwRange.low) + '–' + decimal(wp.requiredKwRange.high) + ' kW · inclusief installatie', qty: 1, unit: 'pakket', priceExVat: wp.investment / 1.21, vatRate: 21, componentKey: 'warmtepomp', lineKind: 'item', vatRefundEligible: false });
    if (selected.indexOf('batterij') >= 0 && bat && bat.product) lines.push({ description: bat.product.name + ' · ' + bat.recommendedKwh + ' kWh', qty: 1, unit: 'pakket', priceExVat: bat.investmentExVat, vatRate: 21, componentKey: 'thuisbatterij', lineKind: 'item', vatRefundEligible: true });
    return lines;
  }
